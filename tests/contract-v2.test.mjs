...
test('v2 accepts a panel with image only and no motion',()=>{
 const m=v2(),panel=m.pages[0].panels[0],motion=panel.motion?.asset;
 panel.text='';delete panel.motion;
 if(motion)m.assets=m.assets.filter(a=>a.id!==motion);
 validate(m);
});
