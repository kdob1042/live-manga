...
export function Reader({manifest,base,preview,onRefreshPreview,refreshing=false}:{manifest:Manifest;base:string;preview?:Preview;onRefreshPreview?:()=>void;refreshing?:boolean}) {
 ...
 const [sidebarOpen,setSidebarOpen]=useState(()=>matchMedia('(min-width: 760px)').matches);
 ...
 useEffect(()=>{if(!active)return;const stopOutside=(event:PointerEvent)=>{const target=event.target; if(!(target instanceof Element))return; if(target.closest('.panel-hit-area,.video-dialog-shell'))return; stop();};document.addEventListener('pointerdown',stopOutside);return()=>document.removeEventListener('pointerdown',stopOutside);},[active]);
