...
export function Reader({manifest,base,preview,onRefreshPreview,refreshing=false}:{manifest:Manifest;base:string;preview?:Preview;onRefreshPreview?:()=>void;refreshing?:boolean}) {
 ...
 const generation=useRef(0),current=useRef<HTMLVideoElement|null>(null),timer=useRef<ReturnType<typeof setTimeout>|undefined>(undefined),surface=useRef<HTMLDivElement|null>(null),dialog=useRef<HTMLDialogElement|null>(null),modalHost=useRef<HTMLDivElement|null>(null),longPress=useRef<{timer:ReturnType<typeof setTimeout>;x:number;y:number}|null>(null),suppressClick=useRef(false),restoreFocus=useRef<HTMLElement|null>(null),sidebarToggle=useRef<HTMLButtonElement|null>(null);
 ...
 return <div className={`reader-shell${sidebarOpen?' sidebar-open':''}`}>
  ...
  <main className="reader-main">
   <div className="reader-status" role="status" aria-live="polite">{status||'▶ のあるコマをタップして再生。長押しで拡大'}</div>
