...
export function Reader({manifest,base,preview,onRefreshPreview,refreshing=false,catalog,workId,format,episodeId}:{manifest:Manifest;base:string;preview?:Preview;onRefreshPreview?:()=>void;refreshing?:boolean;catalog?:PublicationCatalog;workId?:string;format?:'manga';episodeId?:string}) {
 ...
 function stop(message='',closeModal=true) {generation.current++;clearTimeout(timer.current);cancelLongPress();if(current.current){current.current.pause();current.current.removeAttribute('src');current.current.load();current.current.remove();current.current=null;}setActive(null);setInlinePlaying(null);setStatus(message);if(closeModal)closeDialog();}
 ...
 useEffect(()=>{if(sidebarOpen)document.querySelector<HTMLElement>('#reader-sidebar a, #reader-sidebar button')?.focus({preventScroll:true});},[sidebarOpen]);
 ...
 function start(panel:Panel,host:HTMLDivElement,{expanded=false,resumeAt=0}={}) {
  ...
  video.onpause=()=>{if(valid()&&!expanded)setInlinePlaying(null);};
  video.onplaying=()=>{if(!valid()){video.pause();return;}clearTimeout(timer.current);video.style.opacity='1';if(!expanded)setInlinePlaying(panel.id);setStatus(expanded?'拡大再生中':'再生中');};
 ...
 return <div className={`reader-shell${sidebarOpen?' sidebar-open':''}`}>
  ...
  <main className="reader-main">
   ...
   {!preview&&catalog&&workId&&episodeId&&<EpisodeEnd catalog={catalog} workId={workId} episodeId={episodeId}/>}
  </main>
