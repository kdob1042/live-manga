...
async function start() {
  ...
  const loadLegacy = async (release: string | null = new URL(location.href).searchParams.get('release')) => {
    ...
    if (!response.ok) throw Error('作品を取得できません');
    ...
    root.render(<Reader manifest={manifest} base={base} />);
  };
  try {
    const wantsPreview = new URL(location.href).searchParams.has('preview');
    const {
      default: PreviewEntry,
      parsePreviewLocation
    } = wantsPreview ? await import('./PreviewEntry') : {
      default: null,
      parsePreviewLocation: () => null
    };
    const scope = parsePreviewLocation(location.href);
    if (scope && PreviewEntry) {
      const {
        Reader
      } = await import('./reader');
      root.render(<PreviewEntry scope={scope} render={(preview, base, onRefresh, refreshing) => <Reader key={base} manifest={preview.manifest} base={base} preview={preview} onRefreshPreview={onRefresh} refreshing={refreshing} />} />);
      return;
    }
    const route = parseViewerRoute(location.pathname);
    if (route.kind === 'library' && new URL(location.href).searchParams.has('release')) {
      await loadLegacy();
      return;
    }
    if (route.kind === 'library' || route.kind === 'work' || route.kind === 'episode') {
      ...
      if (route.kind === 'library') {
        ...
        root.render(<WorkLibrary catalog={catalog} />);
        return;
      }
      if (route.kind === 'work') {
        ...
        root.render(<WorkLibrary catalog={catalog} focusWorkId={route.workId} />);
        return;
      }
      ...
      if (!response.ok) throw Error('漫画を取得できません');
      const data = await response.text();
      if (data.length > 4 * 1024 * 1024) throw Error('作品データが大きすぎます');
      const manifest = validate(JSON.parse(data)) as Manifest;
      ...
      root.render(<Reader manifest={manifest} base={base} catalog={catalog} workId={route.workId} format="manga" episodeId={route.episodeId} />);
      return;
    }
