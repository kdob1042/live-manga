...
export default function VideoDialog({panel, video, dialogRef, hostRef, onClose}: Props) {
  ...
  useEffect(() => {
    ...
    const metadata = () => { setDuration(Number.isFinite(video.duration) ? video.duration : 0); time(); };
    const volume = () => setMuted(video.muted);
    const play = () => setPlaying(true);
    const pause = () => setPlaying(false);
    const click = () => toggle();
    const events = {timeupdate: time, loadedmetadata: metadata, volumechange: volume, playing: play, pause, click};
    for (const [name, handler] of Object.entries(events)) video.addEventListener(name, handler);
    metadata(); volume(); setPlaying(!video.paused);
    return () => {
      clearTimeout(timer.current);
  ...
  return <dialog ref={dialogRef} className="video-dialog" aria-label={panel ? `${label}の拡大動画` : '拡大動画'}
      ...
      <div className={`modal-ui${visible ? ' is-visible' : ''}`}>
        ...
        <div className="modal-controls" aria-label="動画操作">
          ...
          <label className="modal-seek-label"><span className="sr-only">再生位置</span>
            <input type="range" min="0" max={duration || 1} step="0.01" value={Math.min(position, duration || 1)} aria-label="再生位置"
              aria-valuetext={`${formatTime(position)} / ${formatTime(duration)}`}
              onChange={event => { if (video) { video.currentTime = Number(event.target.value); setPosition(video.currentTime); show(true); } }}/>
          </label>
          <span className="modal-time" aria-live="off">{formatTime(position)} / {formatTime(duration)}</span>
          <button className="modal-mute" type="button" aria-label={muted ? '音声をオン' : 'ミュート'}
            onClick={() => { if (video) { video.muted = !video.muted; setMuted(video.muted); show(true); } }}>{muted ? '音声をオン' : 'ミュート'}</button>
        </div>
      </div>
    </div>
  </dialog>;
